mod admin_routes;
use admin_routes::build_admin_routes_with_options;
pub mod consts;
pub mod conversions;
...
pub async fn init_domain_services_with_pool(
    ...
    let admission_cache: Option<Arc<dyn services::usage::admission::AdmissionCache>> =
        if config.admission_cache.redis_url.is_some() {
            Some(Arc::new(
                services::usage::admission::RedisAdmissionCache::new(&config.admission_cache)
                    .expect("Invalid admission cache configuration"),
            ))
        } else {
            None
        };
    let admission = Arc::new(services::usage::admission::AdmissionCoordinator::new(
        Arc::new(database::repositories::PgAdmissionSnapshotRepository::new(
            database.pool().clone(),
            std::time::Duration::from_millis(config.admission_cache.fallback_deadline_ms),
        )),
        admission_cache,
        config.admission_cache.clone(),
        metrics_service.clone(),
    ));
...
pub fn build_app_with_config_and_options(
    ...
    let usage_state = middleware::UsageState {
        usage_service: domain_services.usage_service.clone(),
        staking_farm_service: domain_services.staking_farm_service.clone(),
        admission: domain_services.admission.clone(),
    };
