...
pub struct AdminAppState {
    ...
    pub credit_adjustment_repository:
        Arc<database::repositories::credit_adjustment::CreditAdjustmentRepository>,
...
pub struct CreateUsageAdjustmentRequest {
    ...
    pub amount: i64,
    pub reason: String,
    /// Caller-stable key used to make uncertain retries safe.
    pub idempotency_key: String,
}

#[derive(Debug, serde::Serialize, utoipa::ToSchema)]
pub struct CreditAllocationReversalResponse {
...
pub struct UsageAdjustmentResponse {
    ...
    pub allocation_reversals: Vec<CreditAllocationReversalResponse>,
}
...
/// Correct attributed usage or write off an unfunded overage (Admin only).
#[utoipa::path(
    post,
...
pub async fn create_usage_adjustment(
    ...
    let adjustment = app_state
        ...
        .create(&CreateCreditAdjustment {
            ...
            changed_by_user_id: Some(admin_user.0.id),
            changed_by_user_email: Some(admin_user.0.email.clone()),
        })
        .await
        .map_err(|error| match error {
            services::common::RepositoryError::NotFound(message) => (
                StatusCode::NOT_FOUND,
                ResponseJson(ErrorResponse::new(message, "not_found".to_string())),
            ),
            ...
            other => {
                error!(%organization_id, error = %other, "Failed to create credit adjustment");
                (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    ResponseJson(ErrorResponse::new(
                        "Failed to create credit adjustment".to_string(),
                        "internal_server_error".to_string(),
                    )),
                )
            }
    ...
    Ok(ResponseJson(UsageAdjustmentResponse {
        ...
        usage_kind: match adjustment.usage_kind {
            AdjustedUsageKind::Inference => "inference",
            AdjustedUsageKind::Service => "service",
        }
        .to_string(),
        adjustment_type: adjustment.adjustment_kind.as_str().to_string(),
        amount: adjustment.amount,
        unfunded_amount_reversed: adjustment.unfunded_amount_reversed,
        reason: adjustment.reason,
        idempotency_key: adjustment.idempotency_key,
        changed_by_user_id: adjustment.changed_by_user_id,
        changed_by_user_email: adjustment.changed_by_user_email,
        created_at: adjustment.created_at,
        allocation_reversals: adjustment
            .allocation_reversals
            .into_iter()
            .map(|reversal| CreditAllocationReversalResponse {
                allocation_id: reversal.allocation_id,
                credit_type: reversal.credit_type,
