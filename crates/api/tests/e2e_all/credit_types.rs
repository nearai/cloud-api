...
/// The explicit legacy-contract rollout converts the old grant instead of
/// layering a postpay ceiling on top of it. Ordinary grant + postpay rows are
/// still allowed; only the reviewed backfill performs this conversion.
#[tokio::test]
async fn test_legacy_contract_grant_conversion_does_not_double_limit() {
    let (server, database) = setup_test_server_with_database().await;
    let org = create_org(&server).await;
    let organization_id = uuid::Uuid::parse_str(&org.id).expect("organization UUID");
    let contract_limit = 10_000_000_000_000_000_i64;

    add_credits_with_type(
        &server,
        &org.id,
        "grant",
        Some("legacy-contract"),
        contract_limit,
        "USD",
        &get_session_id(),
    )
    ...
    let updated = transaction
        .execute(
            ...
            WHERE organization_id = $2
              AND credit_type = 'grant'
              AND effective_until IS NULL
              AND spend_limit = $3
            "#,
            &[&now, &organization_id, &contract_limit],
        )
        .await
        .expect("expire legacy contract grant");
    assert_eq!(updated, 1, "exactly one reviewed legacy grant is converted");
    transaction
        .execute(
            r#"
            INSERT INTO organization_limits_history (
                organization_id, spend_limit, credit_type, source, currency,
                effective_from, changed_by, change_reason
            ) VALUES ($1, $2, 'postpay', 'contract', 'USD', $3,
                      'issue-704-backfill', 'test legacy contract conversion')
            "#,
            &[&organization_id, &contract_limit, &now],
        )
        .await
        .expect("insert postpay ceiling");
    transaction.commit().await.expect("commit conversion");
    ...
    assert_eq!(response.status_code(), 200);
    let balance = response.json::<api::routes::usage::OrganizationBalanceResponse>();
    assert_eq!(balance.spend_limit, Some(contract_limit));
    assert_eq!(balance.credit_limits.len(), 1);
    assert_eq!(balance.credit_limits[0].credit_type, "postpay");
    assert_eq!(balance.credit_limits[0].amount, contract_limit);
    ...
    assert!(history
        .history
        .iter()
        .any(|limit| { limit.credit_type.as_str() == "grant" && limit.effective_until.is_some() }));
    assert!(history.history.iter().any(|limit| {
        limit.credit_type.as_str() == "postpay" && limit.effective_until.is_none()
    }));
}
