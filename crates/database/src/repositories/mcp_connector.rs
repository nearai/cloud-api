use crate::models::{
    McpConnector, McpAuthType, McpConnectionStatus, McpConnectorUsage,
    CreateMcpConnectorRequest, UpdateMcpConnectorRequest, McpBearerConfig,
};
...
impl McpConnectorRepository {
    ...
    pub async fn create(
        ...
        ).await.context("Failed to create MCP connector")?;
        
        debug!("Created MCP connector: {} for organization: {}", id, organization_id);
    ...
    pub async fn get_by_id(&self, id: Uuid) -> Result<Option<McpConnector>> {
        ...
        let row = client.query_opt(
            "SELECT * FROM mcp_connectors WHERE id = $1",
            &[&id],
        ).await.context("Failed to query MCP connector")?;
    ...
    pub async fn list_by_organization(&self, organization_id: Uuid) -> Result<Vec<McpConnector>> {
        ...
        let rows = client.query(
            ...
            "#,
            &[&organization_id],
        ).await.context("Failed to query MCP connectors")?;
    ...
    pub async fn list_active_by_organization(&self, organization_id: Uuid) -> Result<Vec<McpConnector>> {
        let client = self.pool.get().await
            .context("Failed to get database connection")?;
        ...
        let rows = client.query(
            ...
            "#,
            &[&organization_id],
        ).await.context("Failed to query active MCP connectors")?;
    ...
    pub async fn update(&self, id: Uuid, request: UpdateMcpConnectorRequest) -> Result<McpConnector> {
        let client = self.pool.get().await
            .context("Failed to get database connection")?;
        ...
        let row = client.query_one(&query, &params)
            .await.context("Failed to update MCP connector")?;
    ...
    pub async fn delete(&self, id: Uuid) -> Result<()> {
        ...
        let result = client.execute(
            ...
            &[&id],
        ).await.context("Failed to delete MCP connector")?;
    ...
    pub async fn update_connection_status(
        ...
        debug!("Attempting to update connection status for connector {}", id);
        debug!("Status: {:?}, Error: {:?}, Has capabilities: {}", 
               status, error_message, capabilities.is_some());
        ...
        info!("Updated connection status for MCP connector {}: {} (rows affected: {})", id, status_str, rows_affected);
        
        if rows_affected == 0 {
            warn!("No rows updated for connector {} - connector may not exist", id);
        }
    ...
    pub async fn log_usage(
        ...
        ).await.context("Failed to log MCP connector usage")?;
    ...
    pub async fn get_usage_logs(
        ...
        let rows = client.query(
            ...
            "#,
            &[&connector_id, &limit],
        ).await.context("Failed to query usage logs")?;
        
        rows.into_iter()
            .map(|row| self.row_to_usage(row))
            .collect()
    }
