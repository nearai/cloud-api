    ...
    async fn get_usage_history(
        ...
        let entries = logs
            ...
            .map(|log| UsageLogEntry {
                id: log.inner.id,
                organization_id: log.inner.organization_id,
                workspace_id: log.inner.workspace_id,
                api_key_id: log.inner.api_key_id,
                response_id: log.inner.response_id,
                model_id: log.inner.model_id,
                public_name: Some(log.public_name),
                input_tokens: log.inner.input_tokens,
                output_tokens: log.inner.output_tokens,
                total_tokens: log.inner.total_tokens,
                input_cost: log.inner.input_cost,
                output_cost: log.inner.output_cost,
                total_cost: log.inner.total_cost,
                request_type: log.inner.request_type,
                created_at: log.inner.created_at,
            })
            .collect();
