...
impl InferenceProviderPool {
    ...
    pub async fn chat_completion_stream_with_attribution(
        ...
        let first_error = match first_event_timeout {
            Some(budget) => match tokio::time::timeout(budget, peek_first_event).await {
                ...
                Err(_elapsed) => {
                    ...
                    tracing::warn!(
                        ...
                        "Upstream sent no stream event before the first-event deadline"
                    );
